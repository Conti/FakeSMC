			...
			switch (name[0]) {
				case 'T': {
					...
					if (index < cpuid_info()->core_count) {
                        ...
                        mp_rendezvous_no_intrs(read_cpu_diode, &cpu_index);
						
						UInt16 t = tjmax[index] - GlobalThermal[index];
				...
				case 'M': {
                    mp_rendezvous_no_intrs(read_cpu_status, &cpu_index);
                    ...
                    if (strcasecmp(name, KEY_NON_APPLE_PACKAGE_MULTIPLIER) == 0) {
                        switch (cpuid_info()->cpuid_family) {
                            case CPUFAMILY_INTEL_NEHALEM:
                            case CPUFAMILY_INTEL_WESTMERE:
                                value = GlobalStatus[cpu_index] * 10;
                                break;
                                
                            case CPUFAMILY_INTEL_SANDYBRIDGE:
                                value = (GlobalStatus[cpu_index] >> 8) * 10;
                                break;
                        }
                    }
                    else {
                        ...
                        if (index < cpuid_info()->core_count) {
                            ...
                            /*switch (cpuid_info()->cpuid_cpufamily) {
                                case CPUFAMILY_INTEL_NEHALEM:
                                case CPUFAMILY_INTEL_WESTMERE:
                                    value = GlobalStatus[index] * 10;
                                    break;
                                    
                                case CPUFAMILY_INTEL_SANDYBRIDGE:
                                    value = (GlobalStatus[index] >> 8) * 10;
                                    break;
                                    
                                default:
                                {*/
                                    float mult = ((float)(((GlobalStatus[index] >> 8) & 0x1f)) + 0.5f * (float)((GlobalStatus[index] >> 14) & 1)) * 10.0f;
